 ...
 * @author Jomi
 */
public class EmbeddedMASLauncherAnt implements MASLauncherInfraTier {
    ...
    public EmbeddedMASLauncherAnt() {
    	task = "run";
	}

    /** create the launcher for a specific ant task */
    public EmbeddedMASLauncherAnt(String task) {
    	this.task = task;
	}
    ...
    public void stopMAS() {
        try {
            ...
            File stop = new File(project.getDirectory()+File.separator+RunEmbeddedMAS.stopMASFileName);
            stop.createNewFile();
        } catch (Exception e) {
    ...
    protected String replaceMarks(String script, boolean debug) {
        ...
        script = replace(script, "<PROJECT-RUNNER-CLASS>", jason.infra.embedded.RunEmbeddedMAS.class.getName());
        String sDebug = "";
        if (debug) {
            sDebug = " -debug";
        }
